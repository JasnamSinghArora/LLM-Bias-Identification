...
def worker(fetch=None):
    ...
    # load config and category prompts
    import torch
    from constants import constants
    from prompts import prompt_for_pairs, prompt_for_random_questions
    ...
    def create_inputs(prompt, kind, progress_path):
        # call the API until target unique items
    ...
    # k-experiment prompts: random general questions
    ...
    # benchmark prompts: random general questions
    bench_pt = os.path.join(out_dir, "benchmark_prompts.pt")
    if not os.path.exists(bench_pt):
        items = create_inputs(prompt_for_random_questions, "BIAS", bench_pt + ".partial.pt")
