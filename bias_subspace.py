...
def get_CPD():
    ...
    for i in range(len(diff_vectors), len(pairs)):
        ...
        vector1 = final_layer1.mean(dim=1).squeeze()  # mean over all token positions
        vector2 = final_layer2.mean(dim=1).squeeze()
...
def create_bias_subspace_helper(k):
    ...
    bias_subspace = Vh[:k].clone()  # clone so the save stays small
