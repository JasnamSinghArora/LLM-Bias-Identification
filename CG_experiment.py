...
from helper import create_inputs, create_output_from_emb, create_outputs_from_text, get_bias_score_from_emb, get_bias_score_from_text, create_embedding, get_optimized_epsilon, cg_solve
...
y_warm_global = None   
def optimize_with_gradient_ascent(og_emb, current_emb, attention_mask, min_update_per_step=0.05):
    ...
    global y_warm_global
    y_warm = y_warm_global
    ...
    while (True):
        ...
        t0 = time.time()
        _, vjp_fn = vjp(M, current_emb)
        def JTu(u): 
            return vjp_fn(u)[0]
        def Jv(v):  
            return jvp(M, (current_emb,), (v,))[1]
        def JJT_reg(u): 
            return Jv(JTu(u)) + 1e-5 * u
        Jg = Jv(gt)
        max_iteration = 20
        y, cg_iters, cg_res = cg_solve(JJT_reg, Jg, max_iteration, tol=1e-4, x0=y_warm)
        y_warm = y.detach()
        pt = gt - JTu(y)
        t1 = time.time()
        print(f"Semantic Preservation Calculations in {t1 - t0:.3f}s  "
                f"(CG: {cg_iters} iters, relres={cg_res:.2e}, "
                f"||pt||/||gt||={(pt.norm()/gt.norm()).item():.3f})")
        ...
        # clear memory
        del output_vec, coords, bias_score, gt, Jg, y, pt, vjp_fn, JTu, Jv, JJT_reg
        torch.mps.empty_cache()

        if torch.isnan(new_emb).any():
            y_warm_global = y_warm
